...
int menu()
{
	...
	printf("10.Clonar la lista de pasajeros y compararla.\n");
	printf("11.Agregar un pasajero en el indice elegido por el usuario.\n");
	printf("12.Modificar los datos de un pasajero accediendo con el indice.\n");
	printf("13.Ingrese el id del pasajero el cual desea conocer su posicion en la lista.\n");
	printf("14.Limpiar lista de pasajeros.\n");
	printf("15.Eliminar y BackUpear un pasajero con el indice.\n");
	printf("16.Crear una copia de la lista desde el indice A al indice B.\n");
	printf("17. Salir\n");

	validarInt("\n\nIngrese una opcion: ",
			   "\n\nError. Ingrese una opcion valida: ", &opcion, 1, 17);
...
//criterio 1 PUSH, criterio 0 SET,
int controller_varios(LinkedList* pArrayListPassenger, int criterio){
		...
		if (pArrayListPassenger != NULL)
		{
			...
			if(new != NULL){
				...
				if(criterio == 1){
					validarInt("\nIngrese el indice donde desea ubicar el pasajero en la lista: \n",
									"\nError. Ingrese el indice donde desea ubicar el pasajero en la lista: \n", &index, 0, ll_len(pArrayListPassenger));
					ll_push(pArrayListPassenger, index, new);
				}else if (!criterio){
					validarInt("\nIngrese el indice del pasajero que desea cambiar los datos: \n",
									"\nError. Ingrese el indice del pasajero que desea cambiar los datos: \n", &index, 0, ll_len(pArrayListPassenger));
						if(!ll_set(pArrayListPassenger, index, new)){
...
if (pArrayListPassenger != NULL)
		{
		validarInt("\nIngrese el indice del pasajero que desea eliminar y a su vez almacenar sus datos: \n",
				"\nError. ,Ingrese el indice del pasajero que desea eliminar y a su vez almacenar sus datos: \n", &index, 0, ll_len(pArrayListPassenger));
		 new = (Passenger*) ll_pop(pArrayListPassenger, index);

		 		FILE *data = fopen("backUpPop.csv", "a");
		 		if (data == NULL) {
		 			todoOk = 0;
		 			printf("\nError al abrir el archivo\n");
		 		} else {
		 			fprintf(data, "%d,%s,%s,%f,%s,%s,%s\n", new->id,
		 									new->nombre, new->apellido, new->precio,
		 									new->codigoDeVuelo, new->tipoPasajero,
		 									new->estadoDeVuelo);
		 			printf("\nPasajero eliminado y guardado con exito. \n");
		 			todoOk = 1;
		 			fclose(data);
		 		}
		}
...
Passenger *new;
			if(this != NULL){
				if (controller_ListPassenger(this))
						{
							validarInt("\n\nIngrese el id del pasajero que desea conocer su posicion en la lista: \n",
									   "\n\nError ReIngrese un id valido del pasajero que desea conocer su posicion en la lista: \n",
									   &id, 1, ll_len(this));
							...
							if (ll_len(this) > 0)
							{
								for (int i = 0; i < ll_len(this); i++)
								{
									...
									{
										controller_showPassenger(new);
											if(ll_contains(this, new)){
												printf("\nEl pasajero se encontro en la lista y su indice es %d\n", ll_indexOf(this, new));
												}else{
												printf("\nError al buscar el pasajero\n");
												}
											}
										}
									}else{
...
//criterio 0 contains all + clone, criterio 1 subList
...
LinkedList* thisAux;
		if(this != NULL){
			...
			if(!criterio){
				thisAux = ll_clone(this);
				if(ll_containsAll(this, thisAux)){
				printf("\nLos elementos de las listas coindicen.\n");
				}else{
				printf("\nLos elementos de las listas no coinciden.\n");
				}
			}else{
				validarInt("\nIngrese el indice desde el cual desea comenzar a copiar la lista: \n",
						"\nError. Reingrese el indice desde el cual desea comenzar a copíar una lista: \n", &minimo, 0, ll_len(this));
				validarInt("\nIngrese el indice hasta el cual desea copiar la lista: \n",
						"\nError. Reingrese el indice desde el cual desea comenzar a copíar una lista: \n", &maximo, minimo, ll_len(this));
				 thisAux = ll_subList(this, minimo, maximo);
				 if (thisAux != NULL){
